...
**RESOLVED 2026-09-16 (same session):** the Office wall switch was off. JJ switched it on; all four bulbs recovered immediately and were verified after a `hue` entry reload — `sensor.hue_{1,2,3,4}_zigbee_connectivity` = `connected`, `light.hue_{1,2,3,4}` = `on`, `sensor.hue_bridge_zigbee_connectivity` = `connected`. GH #52 closed.
**Diagnostic to keep:** for any `unavailable` Hue entity, un-hide `sensor.<light>_zigbee_connectivity` first. One read separates "HA can't reach the bridge" from "the bridge can't reach the bulb"; the latter is always physical. And never read `light.<room>` as evidence — Hue room groups have no connectivity resource and echo the last commanded state forever.
